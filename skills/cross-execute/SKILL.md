...
3. **写 brief + 派发**：把该任务**完整规格 + 项目画像约定（命令/风格/反模式）+ 反越界边界 + 验收命令**写进 prompt-file，**并明确要求执行者在该 worktree 内 `git commit` 自己的改动**（否则无可 merge 的提交、worktree 也会因 dirty 无法 remove）。经 `external-agent.sh dispatch-write <adapter> <prompt-file> <out-file> <worktree-dir>` 派给外部模型在该 worktree 落地。
