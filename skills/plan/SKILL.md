...
适用档位 **R2 / R3**。在 Flow 流程中的位置：`brainstorm（门）→ 【plan（门）】→ implement → verify → document`。plan 的唯一产物是文档（design + tasks + 必要的图），**不写代码、不建目录、不跑实现**——那是 implement 的事。
...
5. **plan 不碰文件树**：不写代码、不建脚手架、不创建目录。产物仅 `design.md` + `tasks.md`（+ Mermaid 图）。
...
拆解纪律细则、更多正反例见 `references/task-decomposition.md`（写任务前先读）。关键结构配 Mermaid：用 `Skill` 加载 `diagram`，图落 `docs/`。
...
| 「先把目录 / 脚手架建好再规划」 | plan **不碰文件树**；那是 implement |
...
- [ ] 关键结构有 Mermaid 图
- [ ] 没碰代码 / 目录树；产物只有 design + tasks（+ 图）

## 收尾 → 门 → 实现

**可选（高风险面）：跨模型对抗审计划**。人类 gate 前，若项目 `docs/flow/cross-verify` 声明了适配器，把 `design.md` 经 `cross-verify`（`references/external-agent.sh` 派发，对象从 diff 换成方案文本）喂异模型，指令「挑出设计缺陷 / 漏掉的风险 / 更优替代」，结构化回传后纳入 Decisions / Risks-Tradeoffs 复核——审的是**设计层盲点**，不替代用户拍板。不可用则跳过并说明。→ 详见 `cross-verify` 技能。
