...
checklist 全过后，设一道**人类 gate**：把 design + tasks 呈给用户确认（R2/R3 可用 plan mode 让用户批准）。确认后 → 用 `Skill` 工具加载 `implement`，按 tasks 逐项 builder / verifier 对抗推进。完成判定由 verify + Stop hook Oracle（`docs/flow/verify-cmd` 验证门 + `docs/flow/robustness-cmd` B3 健壮性门）裁决，不由 agent 自报。
