...
- **正例**：「`docs/flow/cross-verify` 配了 codex-cli，调外部 verifier 喂 `BASE..HEAD` diff + 对抗指令，回传：`Critical: anchor.go:88 并发下 map 无锁写`（附复现）。复核 diff 确认可复现 → 回 builder 修 → `verify` 跑 `make test-unit` 全绿（fresh 输出附后）。」
