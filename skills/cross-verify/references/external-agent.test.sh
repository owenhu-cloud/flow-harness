...
env GROK_BIN="$TMP/argcodex" GROK_HOME="$GH"      sh "$SUT" healthcheck grok-cli
check 0 $? "grok healthcheck available -> 0"
env GROK_BIN="$TMP/nope"     GROK_HOME="$GH"      sh "$SUT" healthcheck grok-cli
check 3 $? "grok healthcheck missing-binary -> 3"
env GROK_BIN="$TMP/argcodex" GROK_HOME="$TMP/noauth" sh "$SUT" healthcheck grok-cli
check 3 $? "grok healthcheck no-auth -> 3"

env GROK_BIN="$TMP/argcodex" GROK_HOME="$GH"      sh "$SUT" dispatch grok-cli "$PF" "$OF"
check 0 $? "grok dispatch ok -> 0"
grep -q "Major" "$OF"; check 0 $? "grok dispatch 输出含裁决"
env GROK_BIN="$TMP/failcodex" GROK_HOME="$GH"     sh "$SUT" dispatch grok-cli "$PF" "$OF" 2>/dev/null
check 1 $? "grok dispatch 失败 -> 1"
env GROK_BIN="$TMP/nope"     GROK_HOME="$GH"      sh "$SUT" dispatch grok-cli "$PF" "$OF" 2>/dev/null
check 3 $? "grok dispatch 不可用 -> 3"
