...
1. **写进 `verify-cmd` / `robustness-cmd` 的命令必须在本机实跑过**，退出码语义确认无误——没实跑的命令不入 Oracle 燃料。看出来 ≠ 跑得通。
...
```
<能独立判定「完成」的命令，如 npm test，或 go test -v ./... && go build ./...（Go 用 -v：否则 B2 计数门解析不出逐条通过数而休眠）>
```

写下这一行即把本项目**接入独立 Oracle 硬门控**：此后每次试图收尾，Stop hook 都以独立进程跑该命令裁决，非 0 退出即打回。所以它必须先在本机实跑通过再写入（铁律 1）。

`docs/flow/robustness-cmd` —— 给 **Oracle B3 健壮性门**读的、只跑**错误路径/异常场景测试子集**的单行命令（如 `pytest -m robustness`、`go test -run 'Error|Invalid|Boundary|Timeout'`、`npm test -- --grep error`）。探测时按项目的标记/命名/目录约定识别这个子集（无现成约定就采样测试名里的 error/invalid/boundary/timeout/permission 等），本机实跑通过再写入。它治"happy-path 全绿即判完成"：B3 会独立跑它，非 0 或异常路径通过数下降即打回。**项目尚无任何异常路径测试时留空**，并明确告知用户「B3 休眠中，plan 的 Robustness-Cases 落成测试后再回本技能补写」——别拿全量 `verify-cmd` 冒充（那等于没有 B3）。

`docs/flow/coverage-cmd`（+ 可选 `docs/flow/coverage-min` 整数地板 0..100）—— 给 **Oracle B4 覆盖率门**读：跑带覆盖率统计的测试命令。**首选让该命令自带阈值**（如 `pytest --cov --cov-fail-under=80`、`jest --coverage --coverageThreshold=…`），退出码即承载地板、不依赖输出解析——这是最稳的接法。若要靠 `coverage-min` 解析地板，B4 只可靠识别 `total:` / `TOTAL` / `coverage:` 三类总计行：**多包 `go test -cover ./...` 不输出总计行**，会取到最后一包而非整体——改用 `go tool cover -func=cover.out`（含 `total:` 行）或直接用 `-coverpkg` + 自带阈值。它是"删分支/删错误路径测试→覆盖率下降"的代理（抓不到同路径假断言，那需变异）。本机实跑通过再写入；探测不出覆盖率工具就留空。

`docs/flow/mutation-cmd`（**仅记录，不入 Stop 门**）—— 变异测试命令（如 `mutmut run`、`npx stryker run`、`cargo mutants`）。每次收尾跑变异是分钟级、会逼用户关掉整个 Oracle，故**不接 Oracle**；记在画像里供 implement verifier 的 mutation 抽查参照、或人/CI 周期跑。探测到变异工具就记下命令，没有就略。
...
| 没有异常路径测试，就把全量 test 命令也写进 robustness-cmd | 那等于 B3 空转、毫无意义；无异常子集就留空并告知用户 B3 休眠，等 plan 的 Robustness-Cases 落成测试再补，不拿全量冒充。 |
...
- ✅ 探到 `go.mod` → 本机实跑 `go test -v ./... && go build ./...`（**-v 不可省**：让 B2 计数门有逐条 `--- PASS:` 可数，否则 B2 对 Go 休眠），退出 0 → 才写入 `verify-cmd`，project.md 命令旁标「已实跑✓ 来源:清单」。
...
- [ ] 识别了错误路径/异常场景测试子集并实跑后写入 `robustness-cmd`（喂 B3）；无此类测试则留空且告知用户 B3 休眠。
- [ ] 有覆盖率工具则写 `coverage-cmd`（首选自带 `--cov-fail-under` 阈值）+ 可选 `coverage-min`（喂 B4），本机实跑过；有变异工具则记 `mutation-cmd`（仅记录、不接 Oracle）。
