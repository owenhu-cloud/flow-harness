...
- [ ] 1.2 把 `references/adapters.md` 重构为可插拔接口：定义适配器需实现的 `dispatch`/`healthcheck` 两契约 + Codex / Grok 适配器节 + 「接新模型」扩展点；删除与脚本重复的内联命令，改为指向 `external-agent.sh`。（注：原计划的 `codex-mcp` 脚本层适配器键后已去除——与 `codex-cli` 行为相同属误导，统一用 `codex-cli`；真 MCP 是 skill 层 in-session 工具。）
