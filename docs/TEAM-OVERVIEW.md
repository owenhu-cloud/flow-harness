...
Flow 的全部能力以原生 Claude Code 技能（`SKILL.md`）形式提供，按需自动加载，通常无须人工调用。现有 20 支技能，按职能分为五类。
...
- `cross-verify` —— 多模型对抗核心：opt-in 时把独立 verifier/reviewer 升级为异模型/外部 agent（首个 Codex），经派发底座 `external-agent.sh` 做多轮证伪闭环；亦供 `plan`/`brainstorm` 做跨模型决策门。适配器不可用即显式降级回同模型，不接入 Oracle（保确定性）。
- `subagent-driven-development` —— 将任务列表逐条派发至全新子代理，配合双重评审与按难度分配模型。

**模型路由**：按角色定档而非按工具品牌定档。planner/orchestrator、方案审、diff review、verifier 使用最强可用模型；已拆清楚、独立、可机检的 executor 默认低一档。Codex CLI 路径中只读验证使用 `CROSS_VERIFY_EFFORT=high`，写沙箱执行使用 `CROSS_EXECUTE_EFFORT=medium`，两者分离以防为了省执行成本压低评审质量。
...
1. **三支 hook** —— 提供机制层兜底。其中 Stop Oracle 仅在【配置了 `docs/flow/verify-cmd`】时构成代理当轮收尾绕不过的硬约束（未配置则放行、退回纪律级）；bootstrap/reinject 两支是注入路由纪律的 nudge，不强制。Oracle 提高绕过成本、留审计痕迹，**非安全边界**（信任根在仓库内，能 commit 改基线/燃料者仍可绕）。装了插件 ≠ 强保护——用 `flow-doctor` 体检实际接入态。
2. **一组按需加载的技能** —— 提供各流程能力，由代理通过原生 `Skill`、`Task`、`TodoWrite` 及 plan mode 进行编排。**注意：判档/门/builder≠verifier/TDD 等是 SKILL 提示词纪律，hook 无法证明其真发生**，靠 agent 自觉，非机器强制。
...
| 机器级 | `Stop` hook 独立 Oracle，以独立进程运行真实命令裁决 | 配了 `verify-cmd` 时当轮收尾绕不过（未配则放行）；非安全边界 |
...
2. 此后代理每次收尾时，`Stop` hook 自动运行门裁决，代理当轮收尾绕不过（提高绕过成本、留痕，非安全边界）。
