...
**实现与验证** — `implement`（builder + 独立 verifier 对抗）· `systematic-debugging`（四阶段根因调试）· `verify`（真实命令 + 新鲜证据）· `code-review`（独立 reviewer 子代理）· `subagent-driven-development`（串行单任务隔离派发）。
